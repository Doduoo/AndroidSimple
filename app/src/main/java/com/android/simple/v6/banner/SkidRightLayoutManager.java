...
public class SkidRightLayoutManager extends RecyclerView.LayoutManager {
    ...
    // item之间的偏移量
    private int mItemOffset = 200;

    /**
     * Sets true to scroll layout in left direction.
     */
    private boolean isReverseDirection = true;
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (!mHasChild) {
            ...
            mItemViewWidth = (int) (getHorizontalSpace() * mItemHeightWidthRatio);
            mHasChild = true;
        }
    ...
    private void fill(RecyclerView.Recycler recycler) {
        ...
        // item之间的偏移量
        final int baseOffsetSpace = isReverseDirection ? mItemOffset : getHorizontalSpace() - mItemViewWidth;
        ...
        for (int i = bottomItemPosition - 1, j = 1; i >= 0; i--, j++) {
            ...
            boolean isOutOfSpace = isReverseDirection ? remainSpace > getHorizontalSpace() : remainSpace <= 0;
            if (isOutOfSpace) {
        ...
        if (bottomItemPosition < mItemCount) {
            final int start = isReverseDirection ? bottomItemVisibleSize - mItemViewWidth : space - bottomItemVisibleSize;
            layoutInfos.add(new ItemViewInfo(start, 1.0f, offsetPercent, start * 1.0f / space).setIsBottom());
        } else {
...
//        final int scaleFix = (int) (mItemViewWidth * layoutInfo.getScaleXY());
        final int scaleFix = 0;
